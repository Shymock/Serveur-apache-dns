...
class PdoGsb {   		
    ...
    public function __construct($serveur, $bdd, $user, $mdp){
        ...
        // demande que le dialogue se fasse en utilisant l'encodage utf-8
  ...
  */
   ...
   * @param double  le montant
   * @return void rien
  */
    public function creeNouveauFraisHorsForfait($idVisiteur,$mois,$libelle,$date,$montant){      
  	    $dateEn = dateFrancaisVersAnglais($date);
  	    $req = "insert into LigneFraisHorsForfait 
            values(null, :idVisiteur, :mois, :libelle, :dateEn, :montant)";
  	    $cmd = $this->monPdo->prepare($req);
        $cmd->bindValue("idVisiteur", $idVisiteur);
        $cmd->bindValue("mois", $mois);
        $cmd->bindValue("libelle", $libelle);
        $cmd->bindValue("dateEn", $dateEn);
        $cmd->bindValue("montant", $montant, PDO::PARAM_INT);
